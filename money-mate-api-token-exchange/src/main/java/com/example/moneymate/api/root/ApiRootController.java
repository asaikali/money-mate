...
public class ApiRootController {
    ...
    public ApiRootResponse getRoot() {
        ...
        response.add(
            Link.of("/docs/api")
                ...
                .withType("text/markdown")
                .withTitle("Money Mate API brokered access overview")
        );
    ...
    private static final String API_OVERVIEW = """
     ...
     Money Mate is a personal finance aggregation API. This brokered variant expects a
     Bearer token issued by an external identity broker and exposes the same
     hypermedia-driven banking surface without a direct login endpoint.
     ...
     Access to user-specific resources requires a Bearer access token issued by the
     identity broker for this API.

     - This API does not expose a username/password login operation.
     - Discover authentication metadata through the OAuth protected resource
       endpoint exposed by the server.
     - Present the Bearer token on authenticated requests.
     - The API may later exchange that token for downstream banking access.
