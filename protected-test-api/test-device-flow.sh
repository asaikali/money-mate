...
BROKER_BASE_URL="${BROKER_BASE_URL:-http://localhost:9000}"
...
echo "  - identity-broker at $BROKER_BASE_URL"
...
echo "Login with your broker credentials and approve consent."
press_enter
...
while [[ "$attempt" -le "$MAX_TOKEN_ATTEMPTS" ]]; do
  ...
  TOKEN_JSON="$(
    http --ignore-stdin --pretty=none --body --form POST "$BROKER_BASE_URL/oauth2/token" \
      ...
      client_id="$CLIENT_ID" 2>/dev/null || true
  )"
