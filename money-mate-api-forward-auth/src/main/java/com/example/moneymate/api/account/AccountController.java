...
import com.example.moneymate.api.security.ForwardedAuthorizationService;
...
public class AccountController {
    ...
    private final ForwardedAuthorizationService forwardedAuthorizationService;

    public AccountController(ObpClient obpClient, ForwardedAuthorizationService forwardedAuthorizationService) {
        this.obpClient = obpClient;
        this.forwardedAuthorizationService = forwardedAuthorizationService;
    }

    @GetMapping
    public ResponseEntity<AccountCollectionResponse> getAccounts() {
        try {
            String bearerToken = forwardedAuthorizationService.currentBearerToken();

            ObpAccountsResponse accountsResponse = obpClient.getAccounts(bearerToken);
            ObpBanksResponse banksResponse = obpClient.getBanks(bearerToken);
            ...
            log.error("Forwarded bearer token is not available: {}", e.getMessage(), e);
            return ResponseEntity
